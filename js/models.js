  ...
  getHostName() {
    ...
    if (this.url.indexOf("www.") === -1){
      if (this.url.indexOf("//") === -1){
        startPos = 0;
        endPos = this.url.indexOf(".");
      }else{
        startPos = this.url.indexOf("//")+2;
        endPos = this.url.indexOf("."); 
      }
    }else{
      startPos = this.url.indexOf("www.")+4; //get string after "www."
      endPos = this.url.indexOf(".", this.url.indexOf(".")+1); // find the second occurrence of "." for ".com" for example
    }
  ...
  async addStory(user, newStory) {
    try{
      ...
      if(user.loginToken = undefined){
        alert("You are not logged in. You must be logged in to add a story.");
      }
      return null;
    }
  }
